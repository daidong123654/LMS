...
function delete_recycle(id,obj)
{
	if (confirm('确定把该留言永久删除吗？')){   
	window.location.href= '<?php echo site_url('message/delete/id')?>'+'/'+id;
    }
}
function out_recycle(id,obj)
{
	if (confirm('确定把该留言还原吗？')){   
	window.location.href= '<?php echo site_url('message/out_recycle/id')?>'+'/'+id;
    }
}
...
<div id="main-tabs" class="x-tab-panel x-border-panel" style="left: 5px; top: 0px;">
  ...
  <div style="position: relative;"  class="x-panel-bwrap">
	  ...
	  <td  onClick="window.location.href='<?php echo site_url('message/add')?>'"><table style="width: hidden;"  class="button x-btn-wrap x-btn x-btn-text-icon " border="0" cellpadding="0" cellspacing="0" ><tbody><tr>
	  ...
	  <div style="border-left:0 solid #99bbe8; border-bottom:0 solid #99bbe8;" id="content" class="x-panel-body" >
	    <div class="list-div " id="listDiv" style="margin-bottom:0px;overflow:auto">	      
		  ...
		  <tr class="x-grid3-row " >
			...
			<td ><div class="x-grid3-cell-inner ">			
			 <a href='<?php echo site_url('message/reply/id/'.$value['id'])?>' style="text-decoration:none" alt='回复' title='回复'>
			 ...
			 <img src="<?php echo base_url()?>images/out_recycle.gif" border="0" width="16" height="16" alt='还原'>
			 </a>&nbsp;&nbsp;
			 
			 <a href='#' id="in_recycle" style="text-decoration:none"  title='永久删除' onclick="delete_recycle('<?php echo $value['id']?>',this)">
