 ...
 */
 class category_model extends CI_Model
 {
	...
	function find_all_categorys()
	{
        ...
        {
			$this->db->select('COUNT(DISTINCT(bookID)) as total');
        	$query_pro = $this->db->get_where('lib_bookinfo',array('booktype' => $row['id']));
            $row['pro_count'] = 0;
            if ($row_pro = $query_pro->row_array())
            {
                $row['pro_count'] = (int)$row_pro['total'];
	 ...
	 */
	 function create()
	 {
	 	...
	 	$this->db->set('Avail_days',$this->Avail_days);
	 	$this->db->insert('lib_books_cat');
    ...
    function update($id)
    {
		...
		{
          $query = $this->db->get_where('lib_books_cat',array('parent_id' => $row['parent_id']));
	...
	function delete($id)
    {
        ...
        return $this->db->delete('lib_books_cat'); 
    }
	...
	{
        $query = $this->db->get_where('lib_books_cat',array('parent_id' => $id));
        $row = $query->row_array();
		if (!empty($row))
		{
			$this->_is_leaf = 0;
    ...
    function _update_leaf($id,$_is_leaf)
	{
        $this->db->set('is_leaf', $_is_leaf);
        $this->db->where('id', $id);
        return $this->db->update('lib_books_cat');      
	}
	...
	function find_sub_cats($id=0)
	{
		...
		if (!empty($row))
		{
			...
			{
				$this->db->select('id, is_leaf');
	         	$query1 = $this->db->get_where('lib_books_cat',array('parent_id' => $id));
	...
	{		
		...
		$this->db->order_by('path','asc');		   
        $query = $this->db->get('lib_books_cat');
        $rows = array();
        foreach ($query->result_array() as $row)
        {        	
			$this->db->select('COUNT(DISTINCT(bookID)) as total');
